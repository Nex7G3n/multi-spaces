...
class MariaDBConnector(BaseConnector):
    ...
    def execute_sp(self, sp_name, params):
        start_time = time.time()
        try:
            # MariaDB/MySQL usa CALL para procedimientos almacenados
            # Los parámetros se pasan directamente.
            # Para SPs que devuelven resultados, se puede usar un cursor.
            
            # Construir la llamada al SP con placeholders '%s'
            ...
            result = None
            # MariaDB/MySQL puede devolver múltiples conjuntos de resultados o ninguno.
            # fetchone() intentará obtener el primer resultado del primer conjunto.
            if self.cursor.description:
                result = self.cursor.fetchone()
            
            # Consumir cualquier resultado restante para evitar errores con el siguiente comando
            # for _ in self.cursor.stored_results():
            #     pass # Consume todos los resultados
    ...
    def create_tables(self):
        ...
        for query in queries:
            try:
                ...
                print(f"ERROR CONNECTOR al ejecutar query de creación de tabla en MariaDB: {e}")
                self.connection.rollback()
    ...
    def create_stored_procedures(self):
        ...
        DELIMITER ;
        """
        # MariaDB/MySQL usa DELIMITER para procedimientos almacenados.
        # Necesitamos dividir el script y ejecutarlo sin DELIMITER.
        # El conector de Python no necesita el DELIMITER.
        
        # Eliminar DELIMITER y ejecutar el script completo
        sp_query_cleaned = sp_query.replace('DELIMITER //', '').replace('DELIMITER ;', '').strip()
        
        try:
            print(f"DEBUG CONNECTOR: Ejecutando query de creación de SP en MariaDB: {sp_query_cleaned[:100]}...")
            self.cursor.execute(sp_query_cleaned)
            # MariaDB/MySQL no siempre requiere commit después de CREATE PROCEDURE
            # pero es buena práctica para asegurar.
            self.connection.commit()
            print("DEBUG CONNECTOR: Commit realizado para creación de SP.")
        except Exception as e:
            print(f"ERROR CONNECTOR al crear SP en MariaDB: {e}")
            self.connection.rollback()
    ...
    def generate_test_data(self):
        ...
        try:
            ...
            print(f"ERROR CONNECTOR al generar datos de prueba en MariaDB: {e}")
            self.connection.rollback()
    ...
    def generate_invoice(self, client_id: int, staff_id: int, products_json_str: str):
        # Para MariaDB/MySQL, el JSON se pasa como JSON
        # El SP en MariaDB/MySQL usará JSON_EXTRACT para parsearlo.
